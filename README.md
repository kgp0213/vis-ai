...
普通问题直接由模型工具循环处理。涉及多步骤、长时间运行、大量输入、写入交付物或外部操作的请求，会由前台任务状态机监督同一个工具循环；它不会另建一套模型执行路径。状态机负责保留任务目标、当前步骤、已取得的工具证据和验收条件，并决定继续、暂停、请求用户确认或按部分结果交付。

- 对可由当前对话和只读调查确定的信息，系统会直接补全；只有会显著改变交付结果、存在高影响歧义或不可逆风险时，才通过一次一问的交互卡片请求确认。
- 大型工具结果会作为上下文输入事务缓存。缓存引用可在同一复杂任务的后续用户消息中恢复，但缓存本身不代表来源已经完整处理；完整交付仍须通过来源覆盖或产物验证。
- 读取 PDF、Office 文档和图片等二进制输入时，`read_file` 会拒绝将其当作 UTF-8 文本读取，并引导模型使用 `prepare_local_document` 获取可读副本。
- 命令不可用、权限不足、工具超时、输入缓存异常或验收缺少证据时，任务会暂停并显示干预卡片，而不是静默重复尝试。重复验收仍无法获得证据时，系统会如实标记为部分结果并保留恢复现场。
