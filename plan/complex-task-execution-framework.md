...
2026-07-21 增加的 `read_prepared_document` 是普通工具循环中的通用只读 Adapter 边界：PDF、Word、表格、演示文稿和文本都通过同一个 `documentRef + cursor` 契约向当前步骤提供一个有界批次。它不调模型、不自动调度下一批，也不决定任务完成。`foreground-task-supervisor.mjs` 持有 prepared-document 来源覆盖证据，并在完整覆盖任务仍有下一游标时阻止最终完成；不可恢复的模型服务错误同样由监督器暂停并保留原 WorkPlan，而不是启动另一个 Worker 或重复模型请求。
