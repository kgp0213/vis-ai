...
- **回退通用复杂任务状态机**：移除 `foreground-task-supervisor.mjs` 及前台监督接线，模型工具循环（`CacheFirstLoop`）重新成为唯一执行内核。多步骤任务通过轻量 `submit_plan` / `mark_step_complete` / `todo_write` 处理，不再维护独立的任务状态机、步骤调度器或完成守卫。已退役的 `complex-task-*.mjs` 历史文件已从维护树清理，不再进入运行时或默认测试。
...
- **文档任务统一走普通工具循环**：PDF、Word、Excel、PPT、HTML、Markdown、CSV 和文本使用同一套文件工具与模型循环；宿主只负责可读文件绑定、上下文输入缓存和执行回执，不再维护文档专用后台续跑或模型路由。
- **图片附件统一存储与恢复**：原始图片分块写入用户数据目录并按内容去重，会话和排队任务不再保存大段 Base64；宿主按当前模型预算校验、缩放和投递图片，支持通过 `read_media` 重读当前会话附件或局部区域。MCP 图片复用同一链路，音频和视频在 Provider 未验证前只保存附件并明确提示未发送。
