...
按模型 JSON 中的 `agentPolicy.documentPolicy` 分批生成、做确定性保留率检查和独立审校，并将失败限制在当前
...
最终正文由宿主按来源顺序组装并原子写入创建任务时的目标路径，不依赖模型在一次回答中生成整份文件。
每个批次清单记录来源字符数和 SHA-256；写入前再次核对提取数量、来源标记顺序、缺失、重复和意外上下文
标记。PDF 页码选择随任务持久化，程序重启后继续任务不会退回到全文件范围。
