...
export function loadSkillIntegrations(skillsRoot, options = {}) {
  ...
  for (const entry of readdirSync(skillsRoot, { withFileTypes: true })) {
    ...
    try {
      const integration = validateSkillIntegration(readJson(manifestPath), readJson(templatesPath), { expectedId: entry.name });
      integrations.push({ ...integration, ...compatibility(integration, options.runtimeVersions) });
    } catch (error) {
      integrations.push({ id: entry.name, displayName: entry.name, version: null, capabilities: [], templates: [], compatible: false, reason: error.message });
    }
  }
