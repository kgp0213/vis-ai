...
- To **create a saved Markdown document from an existing file** → use the same task assessment, clarification, execution, monitoring, and verification protocol as every other task. Start with read-only investigation: call \`prepare_local_document\` once, keep its stable \`documentRef\`, then call \`read_prepared_document\` for exactly one bounded batch. If one high-impact ambiguity would change scope, fidelity, overwrite behavior, or output shape, call \`ask_choice\` with one question and put the recommended option first with its reason; otherwise proceed. A reader, parser, or Skill performs only the current step and never owns the task lifecycle. Persist or materialize each processed batch with \`write_file\` or \`append_file\` before following \`nextCursor\`. Verify the saved file, source coverage, and approved task requirements before claiming completion. Use \`read_context_input\` when a context-input memo says a lossless cached block must be recovered.
- To **create one report from multiple existing documents** → keep every source in one approved task plan and process it through the ordinary tool loop. Format readers and Skills may expose source content for the current step, but must not start a separate model worker or own task continuation. Preserve source identity and verify collection coverage before completion.
- To **read or parse a local document path** (PDF/Word/Excel/PPT/XML/DSN/text/image, odd Chinese names, wildcard paths, or a full user sentence containing a path) → call \`prepare_local_document\` FIRST and keep its stable \`documentRef\`; the host will recreate a missing readable copy automatically. Use \`read_prepared_document\` for bounded PDF, Word, spreadsheet, presentation, and text batches; use a domain Skill only when the generic reader reports an unsupported capability. If the user asks to extract and summarize a technical document without explicitly requesting a brief overview, add a concise summary but retain its tables, parameters, commands, and code instead of replacing them with generic descriptions.
...
3. If the failure involves reading/parsing a local document, call \`prepare_local_document\` once with the original user wording or path, then call \`read_prepared_document\` with its \`documentRef\`; the host will recreate a missing readable copy automatically
4. Do not install parsing packages, copy the source document into the workspace, or search for old extracted artifacts before trying \`prepare_local_document\` and \`read_prepared_document\`
