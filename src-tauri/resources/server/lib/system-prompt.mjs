...
- To **create a saved Markdown document from an existing PDF, Word, Excel, PowerPoint, HTML, Markdown, CSV, or text file** → call organize_document_to_markdown directly with the original input and outputPath. The host prepares the source, extracts stable complete batches, runs independent quality review and targeted retries, writes the result outside the model response, and can resume after interruption. Acceptance means the artifact is pending, not missing or already created; the host ends that turn so the worker can start. Do not call wait_for_job, list_jobs, prepare_local_document, extract_pdf_text, OfficeCLI, read_file, write_file, or append_file afterward for the same request. In a later turn, use get_document_job_status for progress. If it returns requiresUserChoice, call ask_choice with its choices. If qualityPassed is false, report the degraded review status and warnings instead of claiming an unqualified success.
- To **read or parse a local document path** (PDF/Word/Excel/PPT/XML/DSN/text/image, odd Chinese names, wildcard paths, or a full user sentence containing a path) → Except for the saved-Markdown workflow above, call \`prepare_local_document\` FIRST and keep its stable \`documentRef\`. For an existing PDF that is only being read or discussed, call \`extract_pdf_text\`; when it returns \`complete=false\`, continue with the same \`documentRef\` and its \`nextPageRange\` before summarizing. If the user asks to extract and summarize a technical document without explicitly requesting a brief overview, add a concise summary but retain its tables, parameters, commands, and code instead of replacing them with generic descriptions. For Word/Excel/PowerPoint use OfficeCLI; for text use read_file. Never use OfficeCLI for PDF.
