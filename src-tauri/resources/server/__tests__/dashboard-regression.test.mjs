...
describe("Dashboard 回归护栏", () => {
  ...
  test("千条会话恢复使用 UI 窗口分页，普通输入不触发顶层逐字渲染", () => {
    ...
    assert.match(app, /const autoScrollTokenRef = A2\(0\)/);
    assert.match(app, /const autoScrollFrameRef = A2\(null\)/);
    ...
    assert.match(app, /cancelAnimationFrame\(autoScrollFrameRef\.current\)/);
    assert.match(app, /token !== autoScrollTokenRef\.current/);
