...
describe("Dashboard 回归护栏", () => {
  ...
  test("千条会话恢复使用 UI 窗口分页，普通输入不触发顶层逐字渲染", () => {
    ...
    assert.match(app, /const chatFeedGenerationRef = A2\(0\)/);
    assert.match(app, /const earlierLoadTokenRef = A2\(0\)/);
    assert.match(app, /loadToken === earlierLoadTokenRef\.current/);
    assert.match(app, /!feed\.isConnected/);
