...
async function flushFiles(ents, dirAbs, dirRel, layers, emit) {
  ...
  const stats = await Promise.all(
    accepted.map(
      (e) => stat(join(dirAbs, e.name)).then((s) => ({ mtimeMs: s.mtimeMs, isFile: s.isFile() })).catch(() => null)
    )
  );
...
async function searchBing(query, opts = {}) {
  const topK = Math.max(1, Math.min(10, opts.topK ?? DEFAULT_TOPK));
  const apiKey = opts.bingApiKey;
  if (!apiKey) throw new Error("Bing search requires an API key. Get one from https://portal.azure.com (Bing Search v7, free tier: 1000/month).");
  const url = `${BING_ENDPOINT}?q=${encodeURIComponent(query)}&count=${topK}&mkt=en-US`;
  ...
  if (results.length === 0) {
    if (data.webPages?.totalEstimatedMatches === 0) return [];
...
function registerFilesystemTools(registry, opts) {
  ...
  const allowAllPaths = opts.allowAllPaths ?? (() => false);
