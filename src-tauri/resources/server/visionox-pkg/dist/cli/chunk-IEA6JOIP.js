...
var ImmutablePrefix = class {
  /** Stable across turns; rebuilt only on /new when visionox.md changed on disk. */
  system;
...
`
  ...
  /** "New chat" — drops in-memory messages, archives the on-disk transcript so it survives in Sessions, keeps sessionName so the prefix cache stays warm. Re-runs the system-prompt builder if one was wired (issue #778: visionox.md edits otherwise need a restart). */
  clearLog() {
