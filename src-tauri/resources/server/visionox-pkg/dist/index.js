...
function sanitizeJsonTransportValue(v) {
  if (typeof v === "string") return replaceLoneSurrogates(v);
  if (v && typeof v === "object") {
    if (Array.isArray(v)) return v.map(sanitizeJsonTransportValue);
    const out = {};
    for (const [k2, val] of Object.entries(v)) out[k2] = sanitizeJsonTransportValue(val);
    return out;
  }
  return v;
}
    ...
    try {
      const resp = await fetchWithRetry(
        ...
        `${this.baseUrl}/chat/completions`,
        {
          ...
          body: JSON.stringify(sanitizeJsonTransportValue(this.buildPayload(opts, false))),
          signal
        },
        { ...this.retry, signal }
      );
...
var EN = {
  ...
  errors: {
    contextOverflow: "Context overflow: session history is {requested}, past the active model's prompt limit. Usually a single tool result grew too big. Visionox applies the active model's configured tool-result budget and auto-heals oversized history on session load. If it still overflows, run /new to start fresh, or open /sessions and press [d] to delete this session.",
    contextOverflowTooMany: "too many tokens",
    auth401: "Authentication failed (API 401): {inner}. The selected provider rejected the API key. Check its credentials and endpoint in model settings.",
    balance402: "Provider account unavailable (API 402): {inner}. Check the selected provider's account status and billing settings.",
    badparam422: "Invalid parameter (API 422): {inner}",
    badrequest400: "Bad request (API 400): {inner}",
    deepseek5xxHead: "Model API unavailable ({status}). The provider returned a server error after automatic retries.",
    ...
    deepseek5xxActionRetry: " Try: (1) wait 30s and retry, (2) switch to another configured model, (3) check the selected provider's status page.",
...
var zhCN = {
  ...
  errors: {
    contextOverflow: "\u4E0A\u4E0B\u6587\u6EA2\u51FA\uFF1A\u4F1A\u8BDD\u5386\u53F2\u5DF2\u8FBE {requested}\uFF0C\u8D85\u51FA\u5F53\u524D\u6A21\u578B\u7684 prompt \u4E0A\u9650\u3002\u901A\u5E38\u662F\u5355\u4E2A\u5DE5\u5177\u7ED3\u679C\u592A\u5927\u3002Visionox \u4F1A\u6309\u5F53\u524D\u6A21\u578B\u914D\u7F6E\u7684\u5DE5\u5177\u7ED3\u679C\u9884\u7B97\uFF0C\u5E76\u5728\u4F1A\u8BDD\u52A0\u8F7D\u65F6\u81EA\u52A8\u4FEE\u590D\u8D85\u5927\u5386\u53F2\u3002\u5982\u679C\u4ECD\u7136\u6EA2\u51FA\uFF0C\u8BF7\u8FD0\u884C /new \u91CD\u65B0\u5F00\u59CB\uFF0C\u6216\u5728 /sessions \u4E2D\u5220\u9664\u8BE5\u4F1A\u8BDD\u3002",
    contextOverflowTooMany: "tokens \u6570\u91CF\u8FC7\u591A",
    auth401: "\u8BA4\u8BC1\u5931\u8D25\uFF08API 401\uFF09\uFF1A{inner}\u3002\u5F53\u524D\u670D\u52A1\u5546\u62D2\u7EDD\u4E86 API Key\uFF0C\u8BF7\u5728\u6A21\u578B\u8BBE\u7F6E\u4E2D\u68C0\u67E5\u51ED\u636E\u548C\u63A5\u53E3\u5730\u5740\u3002",
    balance402: "\u670D\u52A1\u5546\u8D26\u6237\u4E0D\u53EF\u7528\uFF08API 402\uFF09\uFF1A{inner}\u3002\u8BF7\u68C0\u67E5\u5F53\u524D\u670D\u52A1\u5546\u7684\u8D26\u6237\u548C\u8BA1\u8D39\u72B6\u6001\u3002",
    badparam422: "\u53C2\u6570\u9519\u8BEF\uFF08API 422\uFF09\uFF1A{inner}",
    badrequest400: "\u8BF7\u6C42\u9519\u8BEF\uFF08API 400\uFF09\uFF1A{inner}",
    deepseek5xxHead: "\u6A21\u578B API \u4E0D\u53EF\u7528\uFF08{status}\uFF09\u3002\u81EA\u52A8\u91CD\u8BD5\u540E\u670D\u52A1\u5546\u4ECD\u8FD4\u56DE\u670D\u52A1\u7AEF\u9519\u8BEF\u3002",
    ...
    deepseek5xxActionRetry: " \u5EFA\u8BAE\uFF1A(1) \u7B49 30 \u79D2\u540E\u91CD\u8BD5\uFF0C(2) \u5207\u6362\u5230\u5176\u4ED6\u5DF2\u914D\u7F6E\u6A21\u578B\uFF0C(3) \u67E5\u770B\u5F53\u524D\u670D\u52A1\u5546\u72B6\u6001\u9875\u3002",
...
function renameSession(oldName, newName) {
  ...
  try {
    ...
    for (const pair of moved.reverse()) {
      try {
        if (existsSync3(pair.target) && !existsSync3(pair.source)) renameSync(pair.target, pair.source);
      } catch {
