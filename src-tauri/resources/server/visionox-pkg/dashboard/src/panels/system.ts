...
export function SystemPanel() {
  useLang();
  const { data, error, loading } = usePoll<HealthData>("/health", 5000, "health");
  if (loading && !data)
    ...
    <div style="display:flex;flex-direction:column;gap:14px">
      ...
      <div class="health-grid">
        ...
        <div class="health-item">
          <div class="lbl">${t("system.sessions")} ${h.sessions.count > 0 ? html`<span class="pill ok">${t("system.ok")}</span>` : html`<span class="pill">${t("system.none")}</span>`}</div>
          <div class="v">${fmtBytes(h.sessions.totalBytes)}</div>
          <div class="meta">${fmtNum(h.sessions.count)} ${t("system.files")}</div>
        </div>

        <div class="health-item">
          <div class="lbl">${t("system.memory")} ${h.memory.fileCount > 0 ? html`<span class="pill ok">${t("system.ok")}</span>` : html`<span class="pill">${t("system.none")}</span>`}</div>
        ...
        <div class="health-item">
          <div class="lbl">${t("system.usageLog")} ${h.usageLog.bytes > 0 ? html`<span class="pill ok">${t("system.ok")}</span>` : html`<span class="pill">${t("system.none")}</span>`}</div>
          <div class="v">${fmtBytes(h.usageLog.bytes)}</div>
          <div class="meta">${h.usageLog.path}</div>
        </div>
      ...
      <div class="card" style="padding:0">
        ...
        <table class="tbl">
          <tbody style="font-size:11.5px">
            <tr><td class="dim" style="padding:5px 14px">${t("system.home")}</td><td class="path">${h.visionoxHome}</td></tr>
