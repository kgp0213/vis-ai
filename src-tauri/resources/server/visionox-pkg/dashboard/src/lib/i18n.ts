...
type TranslationValue = string | readonly string[] | Nested;
type Nested = { [k: string]: TranslationValue };

function get(translations: Nested | undefined, path: string): string | undefined {
  let val: TranslationValue | undefined = translations;
  for (const part of path.split(".")) {
    if (val === undefined || typeof val === "string" || Array.isArray(val)) return undefined;
    val = val[part];
  }
